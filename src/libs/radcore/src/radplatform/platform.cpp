...
class radPlatform : public IRadPlatform
{
    ...
    {   
        //rWarningMsg( m_wmInfo.subsystem != SDL_SYSWM_WINDOWS, "WM info doesn't match the windows subsystem." ); spamming for no reason rn?
    ...
    void LoadIrxModule( const char* pIrxFilenameNoPath, int argSize = 0, char* args = NULL )
    {
        ...
        if ( pIrxFilenameNoPath != NULL )
        {
            ...
            else
            {
                ...
                if( m_IOPMedia != IOPMediaHost )
                {
                    radRef< IRadString > xIString_CdPath;
                    radStringCreate( &xIString_CdPath, RADMEMORY_ALLOC_TEMP  );
                    radRef< IRadString > xIString_CdFilespec;
                ...
                else
                {
                    ...
                    //
                    radRef< IRadString > xIString_HostPath;
                    ...
                    if( loadResult < 0 )
                    {
                        ...
                        //
                        radRef< IRadString > xIString_SNHostPath;
    ...
    radRef< IRadString >           m_xIString_IrxPath;
    radRef< IRadObjectList >       m_xIOl_LoadedIrxModules;
